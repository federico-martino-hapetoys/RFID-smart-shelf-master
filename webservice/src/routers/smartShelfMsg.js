...
router.get('/', async (req, res) => {
  try {
    ...
    const msgs = await SmartShelfMsg.find()
    if (!msgs) {
...
router.get('/findById', async (req, res) => {
  try {
    ...
    const valid = mongoose.Types.ObjectId.isValid(req.query.msgId)
    if (!valid) {
      return res.status(400).send({ error: 'Not a valid ObjectId id for msg' })
    }
    const msg = await SmartShelfMsg.findById(req.query.msgId)
...
router.get('/:msgId', async (req, res) => {
  try {
    ...
    // Check if is a valid MongoDB Id
    const valid = mongoose.Types.ObjectId.isValid(req.params.msgId)
    if (!valid) {
      return res.status(400).send({ error: 'Not a valid ObjectId for msg' })
    }
    const msg = await SmartShelfMsg.findById(req.params.msgId)
